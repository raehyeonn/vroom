...
@SpringBootTest
@ActiveProfiles("dev")
public class FollowPessimisticLockTest {
    ...
    void setUp() {
        target = Member.builder()
            .email("testTarget23@example.com")
            .nickname("테스트타겟23")
    ...
    @Test
    @DisplayName("100명의 사용자가 동시에 팔로우 요청 시 비관적 락 및 성능 테스트")
    void testPessimisticLockingAndConcurrentFollowRequests() throws InterruptedException {
        ...
        for (int i = 0; i < totalRequests; i++) {
            ...
            executorService.submit(() -> {
                try {
                    ...
                    followService.followMember(userDetails, target.getNickname());
                    successCount.incrementAndGet();

                    System.out.println("팔로우 성공 [" + index + "]");
                } catch (Exception e) {
